...
void Console::Start()
{
	...
	if (input[0] == "login")
	{
		...
		System::getInstance().login(input[1], input[2]); // make login check in save file
	}
	...
	else if (input[0] == "view-profile")
	{
		...
		//System::getInstance().getLoggedUser().printInfo(); add printInfo to user
	}
	...
	{
		if (input.getSize() == 1)
			bool b;//get all quizzes from the file
